...
const DECISIONS: [string, string][] = [
  ...
  ["四个入口，各回答一个问题", "今天：我现在怎么样；记录：我经历过什么；洞察：为什么我最近会这样；关怀：我现在可以做什么。"],
  ...
  [
    "特别时期",
    "感情变化、工作压力、家庭烦恼是「今天」里的场景入口，不是独立的功能：记下的内容统一放进「记录」，规律统一出现在「洞察 → 触发因素」。其中的失恋恢复模式不是强制断联：想联系 TA 时，先记录冲动、写下想说的话、看清自己真正需要什么，再由你自己决定。重新联系不会被表达成失败或清零；不判断对方，不替你做决定。",
  ],
  [
    "把身体数据放进来",
    ...
    <div className="space-y-6">
      ...
      <section id="design" className="card-soft scroll-mt-24 px-5 py-6 sm:px-7">
        ...
        <ul className="mt-3 space-y-2 text-sm leading-relaxed text-foreground/85">
          ...
          <li>· 失恋恢复模式里的整理只分析你自己的表达：不判断对方的人格和想法，不说 TA 是否在乎你、会不会复合，不鼓励报复或反复联系，是否联系由你决定。写下的话里出现伤害自己的信号时，同样优先显示求助信息。</li>
      ...
      <section id="privacy" className="card-soft scroll-mt-24 px-5 py-6 sm:px-7">
        ...
        <dl className="mt-5 space-y-5 text-sm leading-relaxed">
          ...
          <div>
            <dt className="font-medium">失恋恢复模式</dt>
            <dd className="mt-1 text-foreground/80">
              可选模块，不需要填写对方的名字。联系冲动、没有发送的话、粘贴进来的消息和给自己的提醒都只保存在这台设备上，不会发送给任何人，也不会出现在导出的 CSV 里；可以在「恢复空间」单独删除。
            </dd>
          </div>
          <div>
        ...
        <div className="mt-6 rounded-2xl bg-secondary/60 px-5 py-5">
          ...
          {confirming && (
            <div className="mt-4 rounded-2xl border border-destructive/40 bg-card px-4 py-4" role="alertdialog">
              <p className="text-sm">
                确定删除全部 {entries.length} 条记录吗？调节记录、睡眠、活动、周期记录、失恋恢复模式和今日卡片的设置也会一起删除，无法恢复，建议先导出。
              </p>
