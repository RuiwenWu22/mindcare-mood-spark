...
type Tab = "sign" | "outfit" | "plan";
const TABS: { key: Tab; label: string }[] = [
  { key: "sign", label: "今日一签" },
  { key: "outfit", label: "今日穿搭" },
  { key: "plan", label: "小计划" },
];
...
export function TodayCard({ onWriteNote }: { onWriteNote: () => void }) {
  ...
  if (!entriesReady || !bodyReady || !dailyReady || !cycle.ready) {
    return (
      <section className={shell} aria-label="今日卡片">
        ...
        <Placeholder className="mt-4 h-7 w-3/4" />
        <Placeholder className="mt-3 h-4 w-1/2" />
      </section>
    );
  }
      ...
      <section className={cn(shell, "bg-accent-soft/60")} aria-label="今日卡片">
        ...
        <p className="mt-2 text-sm leading-relaxed text-foreground/85">
          ...
          <a
            ...
            className="mx-1 inline-flex items-center gap-1 font-medium underline underline-offset-2"
          >
  ...
  if (!state.flipped) {
    return (
      <section className={shell} aria-label="今日卡片">
        <p className="text-xs text-muted-foreground">今日一签</p>
        {todayLog?.sleep ? (
          <button
  ...
  const toggleDone = (item: PlanItem) => {
    ...
    updateState({
      done: done ? state.done.filter((d) => d !== item.id) : [...state.done, item.id],
    });
  ...
  return (
    <section className={shell} aria-label="今日卡片">
      ...
      {/* ---------- 今日一签 ---------- */}
      {tab === "sign" && (
        <div className="animate-rise mt-6" role="tabpanel">
          <div className="flex items-baseline justify-between gap-3">
            <p className="text-xs text-muted-foreground">
              {card.theme ? `${card.theme.symbol} ${card.theme.signName}` : ""}
            </p>
            {profile.sign && (
              <button
          ...
          <p
            className={cn(
              ...
              card.theme ? "mt-3" : "mt-3 font-display text-lg",
            )}
          >
            {card.advice}
          </p>
      ...
      {tab === "outfit" && (
        <div className="animate-rise mt-6" role="tabpanel">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-xs text-muted-foreground">主题色来自星座，衣服来自天气 · 趣味参考</p>
          ...
          {card.outfit.color ? (
            <p className="mt-3 flex items-center gap-2 text-sm">
              <span
                ...
                style={{ backgroundColor: card.outfit.color.hex }}
              />
          ...
          {wStatus === "loading" && (
            <p className="mt-2 text-sm text-muted-foreground">正在查询天气……</p>
          )}
          {wStatus === "error" && (
            <p className="mt-2 text-sm text-muted-foreground">天气暂时获取不到，稍后再看看。</p>
          )}
          {wStatus === "idle" && (
          ...
          {card.outfit.accessory && (
            <p className="mt-2 text-sm text-foreground/85">
              小配饰：{card.outfit.accessory}，给自己一点好心情。
            </p>
          )}
          {profile.city && (
      ...
      {tab === "plan" && (
        <div className="animate-rise mt-6" role="tabpanel">
          <p className="text-xs text-muted-foreground">最多 3 件小事，来自你的记录和今天的天气</p>
          {state.rest ? (
            <div className="mt-3">
              <p className="font-display text-lg">好的，今天就好好休息。</p>
              <button
                onClick={() => updateState({ rest: false })}
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {item.action &&
                            (item.action.external ? (
                              ...
                              <Link
                                to="/care"
                                className="mt-1.5 inline-flex text-xs underline underline-offset-2"
                              >
                                {item.action.label}
                              </Link>
                            ))}
